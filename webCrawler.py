...
#valid = has 100% cotton
    ...
    if tfIdf == 1:
        ...
        i = 0
        for term in terms:
            idf.append(math.log(D / dfi[i], 10))
            i = i + 1
...
tp2, fp2, length = CrawlForIngredients("https://www.asos.com/search/?q=men+shirt+100%25+cotton","100% Cotton",1, 0, "")

#assume 25% not retrived 
fn = math.floor(length * 0.25)
precision = (tp1 + tp2) / (tp1 + fp1 + tp2 + fp2)
recall = (tp1 + tp2) / (tp1 + fn + tp2 + fn)
...
tp2, fp2, length = CrawlForIngredients("https://www.asos.com/search/?q=shoe%20lining%3A%20100%25%20polyurethane","100% Polyurethane",2, 0, "")
